...
module.exports = {
  ...
  determineChoice: determineChoice,
  reportInvalidTheme: reportInvalidTheme
};
...
/**
 * Determines an option based on the command line flags, theme options, and user prompting
 * @param  {Object}   choiceOptions Object containing the possible options
 * @param  {Function} callback      Callback to invoke with determined choice
 */
function determineChoice(choiceOptions, callback) {
  if (choiceOptions.cliFlag) {
    callback(choiceOptions.cliFlag);
  } else {
    if (choiceOptions.choices.length === 1) {
      callback(choiceOptions.choices[0]);
    } else {
      inquirer.prompt({
        type: 'list',
        name: 'choice',
        message: 'Which ' + choiceOptions.choiceName + ' will you use?',
...
/**
 * Ask user if they want to file an issue about the invalid theme
 * @param  {String}  theme        Name of theme
 * @param  {String}  errorMessage Error message to log
 **/
function reportInvalidTheme(theme, errorMessage) {
