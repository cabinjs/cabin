  ...
  async.series([
    ...
    function (step) {
      if (cliOptions.local) {
        ...
        downloadTheme(cliOptions, function (tmpThemeFolder) {
          themeSourceFolder = tmpThemeFolder;
          step();
        });
      }
    },
    ...
    if (!cliOptions.local) {
      // Delete temporary theme source folder
...
function downloadTheme(cliOptions, callback) {
  ...
  request('https://github.com/' + cliOptions.theme + '/archive/master.zip')
