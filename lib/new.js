...
function getThemeConfig(theme, themeSourceFolder) {
  ...
  if (packageJSON.devDependencies && packageJSON.devDependencies.cabin) {
    ...
    invalidThemePrompt(theme, 'Missing Cabin dependency in package.json.');
    return;
  }

  var thisCabinVersion = JSON.parse(fs.readFileSync('package.json', 'utf8')).version;
  ...
  if (themesCabinVersion.split('.')[1] !== thisCabinVersion.split('.')[1]) {
    if (parseInt(themesCabinVersion.split('.')[1], 10) > parseInt(thisCabinVersion.split('.')[1], 10)) {
      console.log('\nTheme compatible with newer Cabin version, please update Cabin with the following command:\n\n'.red +
              'npm install -g cabin'.blue);
      process.exit(1);
      return;
    } else {
      invalidThemePrompt(theme, 'Theme compatible with older Cabin version.');
      return;
    }
  }
