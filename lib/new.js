...
function generateProject(siteConfig, themeSourceFolder, callback) {
  ...
  async.series([
    ...
    function (step) {
      ...
      // By default there is no deployment credentials file to be added to the .gitignore
      siteConfig.deployCredentialsFile = '';

      // Generate config files needed for certain deployment tools
      if (siteConfig.deployTask === 's3') {
        siteConfig.deployCredentialsFile = 'grunt-aws.json';
        ...
        siteConfig.deployCredentialsFile = '.ftpass';
        utils.safeWriteFile(siteConfig.siteName + '/' + siteConfig.deployCredentialsFile, JSON.stringify({
          key: {
