      ...
      function (callback) {
        if (!options.preprocessor) {
          utils.choicePrompt('Which CSS preprocessor will you use?', themeData.style, function (result) {
            options.preprocessorChoice = result;
            options.preprocessor = userOptions.style[result].taskName;
            callback();
          });
        } else callback();
      },
      ...
      function (callback) {
        if (!options.templateLang) {
          utils.choicePrompt('Which template language will you use?', themeData.template, function (result) {
            options.templateLangChoice = result;
            options.templateLang = userOptions.template[result].taskName;
            callback();
          });
        } else callback();
      },
