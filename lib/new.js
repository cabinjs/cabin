  ...
  async.series([
    ...
    function (step) {
      ...
      getUserChoices(cliOptions, themeConfig, function (userChoices) {
        ...
        siteConfig.gruntPages = themeConfig.gruntPages;
    ...
    function (step) {
      ...
      if (!cliOptions.CSSPreprocessor) {
        if (themeConfig.CSSPreprocessor.length === 1) {
          ...
          siteConfig.setPreproccesor(themeConfig.CSSPreprocessor[0]);
          step();
        } else {
          inquirer.prompt({
            type: 'list',
...
function generateProject(siteConfig, themeSourceFolder, callback) {
  ...
  async.series([
    ...
    function (step) {
      ...
      var gruntPagesConfigTemplate = prettyStringify(siteConfig.gruntPages, { initialIndent: 2 });

      // Render grunt-pages config with option data
      siteConfig.gruntPages = _.template(gruntPagesConfigTemplate, siteConfig);
