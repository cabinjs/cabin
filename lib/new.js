...
module.exports = function (options, callback) {
  getThemeOptions(options.theme, function (err, themeData) {
    ...
    async.series([
      ...
      // the choices are already passed in don't prompt (used for testing)
      function (callback) {
...
function downloadTheme(options, callback) {
  ...
  req.on('end', function () {
    ...
    async.eachSeries(zip.getEntries(), function (zipEntry, callback) {
      ...
      // Only copy readme and files in src and posts folders
      if (root === 'src' || root === 'posts' || root === 'README.md') {
        ...
        // jade, copy *.jade, always copy files that containg '.syntax.'
        // because those are synax highlighting files
